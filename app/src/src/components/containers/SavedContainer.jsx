...
class SavedContainer extends Component
{
    ...
    fetchItems = async () => {
        ...
        for (let i = 0; i < endpoints.length; i++)
        {
            try {
                responseDataList[i] = (
                    await endpoints[i].saved(nextPages[i])
                ).data;
            } catch (error) {
    ...
    renderItem = ({ item }) => {
        ...
        return(
            <Card style={styles.row}>
                ...
                <ImageBackground source={testImage} style={{paddingBottom: 95}} imageStyle={{borderTopLeftRadius: 20, borderTopRightRadius: 20}}>
                    <Card.Content>
        ...
        return (
            <View>
                {(this.state.items.length) ?
                    ...
                    :
                    <View>
                        <Paragraph style={styles.empty}>Sorry, No items to display!!</Paragraph>
                    </View>
                }
            </View>
        );
