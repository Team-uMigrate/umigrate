...
class CustomUser(AbstractUser, GenericPhotoModel):
    ...
    pronouns = models.PositiveSmallIntegerField(
        choices=Choices.PRONOUN_CHOICES, default=0, blank=True
    )
    ...
    profile_photo = models.ImageField(
        upload_to="images/user_profile_photos", blank=True
    )
    background_photo = models.ImageField(
        upload_to="images/user_background_photos", blank=True
    )
    region = models.PositiveSmallIntegerField(choices=Choices.REGION_CHOICES, default=0)
    datetime_created = models.DateTimeField(auto_now_add=True)

    # User settings field
    notification_privacy = models.PositiveSmallIntegerField(
        choices=Choices.NOTIFICATION_PRIVACY_CHOICES, default=0
    )
    allow_location = models.BooleanField(default=False)
    currency = models.PositiveSmallIntegerField(
        choices=Choices.CURRENCY_CHOICES, default=0
    )
    language = models.PositiveSmallIntegerField(
        choices=Choices.LANGUAGE_CHOICES, default=0
    )
    dark_theme = models.BooleanField(default=False)
    connected_users = models.ManyToManyField(
        to="self", related_name="connected_user_set", blank=True
    )
    blocked_users = models.ManyToManyField(
        to="self", related_name="blocked_user_set", blank=True
    )
