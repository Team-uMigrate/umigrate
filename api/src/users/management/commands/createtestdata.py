...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for i in range(ITEM_COUNT):
            ...
            ad = AdFactory(liked_users=random_users(users), tagged_users=random_users(users),
                           saved_users=random_users(users), creator=users.get(id=random.randint(1, USER_COUNT)))

            event = EventFactory(liked_users=random_users(users), tagged_users=random_users(users),
                                 saved_users=random_users(users), creator=users.get(id=random.randint(1, USER_COUNT)),
                                 interested_users=random_users(users), attending_users=random_users(users))

            job = JobFactory(creator=users.get(id=random.randint(1, USER_COUNT)))

            listing = ListingFactory(liked_users=random_users(users), tagged_users=random_users(users),
                                     saved_users=random_users(users), creator=users.get(id=random.randint(1, USER_COUNT)))

            room = RoomFactory(members=random_users(users), creator=users.get(id=random.randint(1, USER_COUNT)))

            for j in range(rand_int1):
                message = MessageFactory(liked_users=random_users(users), tagged_users=random_users(users),
                                         creator=users.get(id=random.randint(1, USER_COUNT)), room=room)

            poll = PollFactory(liked_users=random_users(users), tagged_users=random_users(users),
                               saved_users=random_users(users), creator=users.get(id=random.randint(1, USER_COUNT)))
            ...
            post = PostFactory(liked_users=random_users(users), tagged_users=random_users(users),
                               saved_users=random_users(users), creator=users.get(id=random.randint(1, USER_COUNT)))

            postings = [ad, event, listing, poll, post]

            for x in range(rand_int1):
                comment = CommentFactory(liked_users=random_users(users), tagged_users=random_users(users),
                ...
                for y in range(rand_int2):
                    reply = ReplyFactory(liked_users=random_users(users), tagged_users=random_users(users),
                                         saved_users=random_users(users),
