...
class UserTestCase(APITestCase):
    ...
    def test_connect_user(self):
        ...
        self.assertEqual(
            len(CustomUser.objects.get(id=2).connected_users.filter(id=1)), 0
        )

    def test_block_user(self):
        block_data = {"block": True, "id": 1}
        ...
        self.assertEqual(
            len(CustomUser.objects.get(id=1).blocked_users.filter(id=1)), 1
        )

        obj = CustomUser.objects.get(id=2)
        obj.blocked_users.add(1)

        unblock_data = {"block": False, "id": 2}

        unblock_response = self.client.post(
            "/api/users/block", unblock_data, format="json"
        )
        self.assertEqual(unblock_response.status_code, status.HTTP_200_OK)
        self.assertEqual(unblock_response.data, unblock_data)
        self.assertEqual(
            len(CustomUser.objects.get(id=2).blocked_users.filter(id=1)), 0
        )
