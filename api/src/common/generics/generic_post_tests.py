...
class GenericPostTestCase:
    ...
    def test_create(self):
        response = self.api_client.post(
            f"/api/{self.resource_name}/", self.create_data, format="json"
        )
        ...
        for key in self.create_data:
            if key in self.ignored_keys:
                ...
                self.assert_equal(
                    float(self.create_data[key]), float(response_dict[key])
                )
            else:
    ...
    def test_retrieve(self):
        ...
        for key in response_dict:
            if key in self.ignored_keys:
                ...
                self.assert_equal(
                    obj[key].strftime("%Y-%m-%dT%H:%M:%S.%fZ"), response_dict[key]
                )
            else:
    ...
    def test_update(self):
        response = self.api_client.put(
            f"/api/{self.resource_name}/1", self.update_data, format="json"
        )
        self.assert_equal(response.status_code, status.HTTP_200_OK)
        ...
        for key in self.update_data:
            if key in self.ignored_keys:
                ...
                self.assert_equal(
                    float(self.update_data[key]), float(response_dict[key])
                )
            else:
    ...
    def test_like(self):
        like_data = {"like": True, "id": 1}

        like_response = self.api_client.post(
            f"/api/{self.resource_name}/like", like_data, format="json"
        )
        ...
        unlike_data = {"like": False, "id": 2}

        unlike_response = self.api_client.post(
            f"/api/{self.resource_name}/like", unlike_data, format="json"
        )
...
class GenericCommentTestCase:
    ...
    def test_create(self):
        data = {"content": "My comment", self.parent_name: 1}

        response = self.api_client.post(
            f"/api/{self.resource_name}/comments/", data, format="json"
        )
        self.assert_equal(response.status_code, status.HTTP_201_CREATED)
    ...
    def test_update(self):
        data = {"content": "My new comment", self.parent_name: 1}
        response = self.api_client.put(
            f"/api/{self.resource_name}/comments/1", data, format="json"
        )
        self.assert_equal(response.status_code, status.HTTP_200_OK)
        self.assert_equal(response.data[self.parent_name], 1)
    ...
    def test_like(self):
        ...
        like_response = self.api_client.post(
            f"/api/{self.resource_name}/comments/like", like_data, format="json"
        )
        self.assert_equal(like_response.status_code, status.HTTP_200_OK)
        self.assert_equal(like_response.data, like_data)
        self.assert_equal(len(self.model.objects.get(id=1).liked_users.filter(id=1)), 1)
        ...
        unlike_response = self.api_client.post(
            f"/api/{self.resource_name}/comments/like", unlike_data, format="json"
        )
        self.assert_equal(unlike_response.status_code, status.HTTP_200_OK)
        self.assert_equal(unlike_response.data, unlike_data)
        self.assert_equal(len(self.model.objects.get(id=2).liked_users.filter(id=1)), 0)
